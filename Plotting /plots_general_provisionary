...
# The corrected-output phase-space maps use the same crossing-angle Lorentz
# boost as the standalone momentum-modification script. The transformation is
# applied only in memory while filling IPC theta-pT histograms; it never changes
# the source HDF5 files or any spatial/counting analysis.
FULL_CROSSING_ANGLE_MRAD = 30.0
BOOST_SIGN_X = +1
CROSSING_HALF_ANGLE_RAD = 0.5 * FULL_CROSSING_ANGLE_MRAD * 1.0e-3
CROSSING_BETA_X = BOOST_SIGN_X * np.sin(CROSSING_HALF_ANGLE_RAD)
CROSSING_GAMMA = 1.0 / np.sqrt(1.0 - CROSSING_BETA_X**2)
...
# Use a stronger, foreground style for the rectangular overlay on 2D density
# maps. The 1D boundary lines retain their existing appearance.
BOX_RECTANGLE_KW = {
    **BOX_LINE_KW,
    "linewidth": 2.2,
    "zorder": 20,
...
# Keep the rectangle slightly inside the visible plotting area rather than
# exactly underneath the axes spines.
BOX_RECTANGLE_VIEW_PADDING = 0.025
...
def phase_space_momentum(
    chunk: ParticleChunk,
    apply_crossing_boost: bool,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return momentum arrays used only for IPC phase-space histograms.

    For the original analysis, the stored SI momentum components are returned
    unchanged. For the corrected output, px is Lorentz-boosted along +x using
    the configured half crossing angle, while py and pz remain unchanged.

    No values in ``chunk`` or in the source HDF5 diagnostics are modified.
    """
    px_si = chunk.px_si
    py_si = chunk.py_si
    pz_si = chunk.pz_si

    if not apply_crossing_boost:
        return px_si, py_si, pz_si

    electron_mc = ELECTRON_MASS_KG * CLIGHT

    # E/c in SI momentum units:
    # E/c = sqrt((m_e*c)^2 + px^2 + py^2 + pz^2)
    ...
    # px' = gamma * (px + beta_x * E/c)
    boosted_px_si = CROSSING_GAMMA * (
        px_si
        + CROSSING_BETA_X * energy_over_c
    )

    return boosted_px_si, py_si, pz_si
...
def second_pass(
    ...
    phase_y_edges,
    correct_outside: bool = False,
    boost_ipc_phase_space: bool = False,
    correction_stats: (
        dict[tuple[str, str], dict[str, int]]
        | None
    ) = None,
    ...
    for _info, chunk in iter_chunks(
        files,
        (
            "corrected boosted phase-space histogram pass"
            if correct_outside and boost_ipc_phase_space
            else (
        ...
        if chunk.species in IPC_SPECIES:
            (
                ...
                phase_py_si,
                phase_pz_si,
            ) = phase_space_momentum(
                chunk,
                apply_crossing_boost=boost_ipc_phase_space,
            )
...
def draw_box_rectangle(
    ...
    x_padding = BOX_RECTANGLE_VIEW_PADDING * (
        new_x1 - new_x0
    )
    y_padding = BOX_RECTANGLE_VIEW_PADDING * (
        new_y1 - new_y0
    )
...
def plot_density(
    ...
    # Draw the simulation-box projection after the density mesh so the dashed
    # magenta rectangle remains visible above populated bins.
    if show_box:
        draw_box_rectangle(
            ax,
            axis_x,
        ...
        for group in PLOT_GROUPS:
            ...
            for a, b in PLANES:
                ...
                plot_density(
                    ...
                    dpi,
                    # Show the simulation-box projection for every ordinary
                    # 2D position-density map, including the zoomed IPC maps.
                    show_box=True,
                )
...
def main() -> None:
    ...
    with (
        ...
        handle.write(
            ...
            "their original momentum direction to the first rectangular-box "
            "intersection. Inside-particle positions are unchanged.\n"
        )
        handle.write(
            "Only corrected-output IPC phase-space histograms apply an "
            "in-memory x-directed Lorentz boost to px. Spatial histograms, "
            "counts, weights, creation records, and source HDF5 files remain "
            "unchanged by the momentum transformation.\n"
        )
        handle.write(
            f"full_crossing_angle_mrad={FULL_CROSSING_ANGLE_MRAD}\n"
        )
        handle.write(
            f"boost_sign_x={BOOST_SIGN_X}\n"
        )
        handle.write(
            f"beta_x={CROSSING_BETA_X:.16g}\n"
        )
        handle.write(
            f"gamma={CROSSING_GAMMA:.16g}\n"
        )
    ...
    for iteration in iterations:
        ...
        generate_plots(
            ...
            phase_label=(
                corrected_iteration_label
                + " — IPC x-boosted phase space "
                + f"({FULL_CROSSING_ANGLE_MRAD:g} mrad full angle)"
            ),
        )
